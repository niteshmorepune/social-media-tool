...
| `META_ADS_ACCESS_TOKEN` / `META_AD_ACCOUNT_ID` / `META_ADS_HOUSE_CLIENT_ID` / `META_PAGE_ID` / `META_GRAPH_API_VERSION` | Meta Ads Manager creative push (house account only, see `lib/meta-ads.ts`) |
| `GOOGLE_ADS_CLIENT_ID` / `GOOGLE_ADS_CLIENT_SECRET` / `GOOGLE_ADS_REFRESH_TOKEN` / `GOOGLE_ADS_DEVELOPER_TOKEN` / `GOOGLE_ADS_LOGIN_CUSTOMER_ID` / `GOOGLE_ADS_CUSTOMER_ID` / `GOOGLE_ADS_HOUSE_CLIENT_ID` / `GOOGLE_ADS_API_VERSION` | Google Ads paused-draft push (house account only, see `lib/google-ads.ts`) |
...
- **AI Ad Review** (`lib/ad-review.ts`, `POST /api/content/[id]/review-ad`): on-demand second-pass Claude critique of already-generated ad copy (either platform) for persuasive/strategic quality — distinct angles, CTA strength, value-prop clarity — never compliance (that's `ad-copy-policy.ts`'s job, kept separate so the two checks never contradict each other). Cached on `Content.adReviewScore`/`adReviewFeedback`/`adReviewedAt`. **Critique-only, never rewrites** — same "AI drafts/advises, human decides" shape as Humanize & Check; the team applies feedback by hand or via Regenerate with Direction.
- **Push to Meta Ads Manager** (`lib/meta-ads.ts`, `POST /api/content/[id]/push-to-meta`): APPROVED Meta ad copy only, house account only (`META_ADS_HOUSE_CLIENT_ID` gate). Creates ONLY an Ad Creative object — no Campaign/AdSet/Ad, so it structurally cannot touch budget/targeting/schedule/activation.
- **Push to Google Ads** (`lib/google-ads.ts`, `POST /api/content/[id]/push-to-google`): APPROVED Google ad copy only, house account only (`GOOGLE_ADS_HOUSE_CLIENT_ID` gate). Google's API has no standalone-creative equivalent — a Responsive Search Ad can only exist inside an Ad Group inside a Campaign inside a Budget — so this creates its own dedicated Campaign+Budget+AdGroup+Ad per push, atomically via the compound `googleAds:mutate` endpoint. Guardrails compensating for the bigger blast radius than Meta: Campaign/AdGroup/Ad all independently created PAUSED; budget (₹100/day placeholder) and CPC bid (₹5 placeholder) are hardcoded constants, never derived from request input; Manual CPC bidding (no automated strategy); campaign name prefixed `⚠️ DRAFT – DO NOT ACTIVATE –`; no geo/language targeting set (won't meaningfully serve without it, which is itself a forcing function to review before activating). **Real residual risk, unlike Meta's push**: this design CAN spend if someone unpauses all three objects without reviewing the placeholder budget/bid/missing targeting first — the guardrails make that unlikely and loud, not impossible.
