...
const StatusBadge = ({ label, color }: { label: string; color: string }) => (
  <span className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full ${color}`}>{label}</span>
)
...
const sections = [
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          This platform manages the full lifecycle of social media content for your clients — from
          briefing through AI generation to client approval and export.
        </p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          A <strong>Client</strong> is a business you manage social media for. Every brief and piece
          of content belongs to a client.
        </p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        <p>
          A <strong>Brief</strong> defines what content to create for a client in a given month. You
          pick the platforms, content types, and goals once — then generate everything from it.
        </p>
        <Step n={1} text='Go to Briefs → "New Brief". Select the client and the scheduled month.' />
        ...
        <div className="border-t border-gray-100 pt-4">
          ...
          <div className="space-y-2">
            {[
              { type: 'Image', desc: 'Single static image with caption. Generated via Flux-1.1-pro.' },
              { type: 'Video', desc: 'Short-form video clip. Thumbnail generated first, then RunwayML renders the video (takes ~2 min).' },
              { type: 'Carousel', desc: 'Multi-slide post. Each slide gets its own image — generation takes longer.' },
            ].map(item => (
              <div key={item.type} className="flex gap-3">
                <span className="text-xs font-semibold bg-gray-100 text-gray-700 px-2 py-0.5 rounded shrink-0 mt-0.5 h-fit">{item.type}</span>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          The <strong>Generate</strong> page is where AI creates the actual content from your briefs.
        </p>
        ...
        <Note text="Video generation (RunwayML) runs asynchronously and may take 1–3 minutes. The status indicator will update automatically — you do not need to refresh the page." />
        ...
        <div className="border-t border-gray-100 pt-4">
          ...
          <div className="space-y-2">
            {[
              { label: 'None', color: 'bg-gray-100 text-gray-600', desc: 'No media generated yet.' },
              { label: 'Generating', color: 'bg-blue-100 text-blue-700', desc: 'Pipeline running — wait for it to complete.' },
              { label: 'Ready', color: 'bg-green-100 text-green-700', desc: 'Media available for preview and download.' },
              { label: 'Failed', color: 'bg-red-100 text-red-700', desc: 'Pipeline error — use Retry Media.' },
            ].map(item => (
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          The <strong>Approvals</strong> page shows all content awaiting a decision. Clients review
          via their own portal; you action their feedback here.
        </p>
        ...
        <div className="border-t border-gray-100 pt-3">
          ...
          <div className="space-y-2">
            {[
              { label: 'Pending', color: 'bg-gray-100 text-gray-700', desc: 'Awaiting client review.' },
              { label: 'Approved', color: 'bg-green-100 text-green-700', desc: 'Client has approved — ready to schedule.' },
              { label: 'Revision Requested', color: 'bg-yellow-100 text-yellow-700', desc: 'Client left a comment requesting changes.' },
              { label: 'Rejected', color: 'bg-red-100 text-red-700', desc: 'Client rejected — regenerate or discuss.' },
            ].map(item => (
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          Clients log in at <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">/portal</code> with their own credentials (CLIENT role). They see only their own content — no other client data is visible.
        </p>
        <Step n={1} text='Create a CLIENT-role user via Team → "New Member" and assign them to the correct client.' />
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          The <strong>Calendar</strong> gives a month-view of all scheduled content across clients and platforms.
        </p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          The <strong>Export</strong> page lets you download content data for reporting or handoff.
        </p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>
          <strong>Admin only.</strong> The Team page manages who has access to the platform.
        </p>
        <div className="space-y-2">
          {[
            { role: 'ADMIN', desc: 'Full access — clients, briefs, generate, approvals, team management, export.' },
            { role: 'TEAM', desc: 'Full access except Team management (cannot create/delete users).' },
            { role: 'CLIENT', desc: 'Portal-only access — can only view and approve their own content.' },
          ].map(item => (
            <div key={item.role} className="flex gap-3">
              <span className="text-xs font-semibold bg-gray-100 text-gray-700 px-2 py-0.5 rounded shrink-0 mt-0.5 h-fit">{item.role}</span>
              <p className="text-gray-600">{item.desc}</p>
            </div>
          ))}
        </div>
        <Step n={1} text='Go to Team → "New Member". Enter name, email, password, and role.' />
